...
# Multi Group A: Pristine Charm - Crystal Comb
    ...
    max_mga = 1.3
    player_mga = ValueToMulti(account.sneaking.pristine['Crystal Comb'].value)
...
def _calculate_master_classes_grimoire_bone_sources(account):
    ...
    account.grimoire['Bone Calc'] = {
        'mga': ValueToMulti(account.sneaking.pristine['Glimmerchain'].value),
...
def _calculate_master_classes_compass_dust_sources(account):
    ...
    account.compass['Dust Calc'] = {
        ...
        'mgc': ValueToMulti(account.sneaking.pristine['Twinkle Taffy'].value),
        'mgd': ValueToMulti(
...
def _calculate_master_classes_tesseract_tachyon_sources(account):
    ...
    account.tesseract['Tachyon Calc'] = {
        ...
        'mgc': ValueToMulti(account.sneaking.pristine['Mystery Fizz'].value),
...
def _calculate_w1_stamps(account):
    ...
    account.exalted_stamp_multi = ValueToMulti(
        ...
        + account.sneaking.pristine['Jellypick'].value
    ...
    for stamp_name, stamp in account.stamps.items():
        try:
            account.stamps[stamp_name].total_value = (
                ...
                * (ValueToMulti(account.sneaking.pristine['Liqorice Rolle'].value) if stamp.stamp_type != 'Misc' else 1)
...
def _calculate_w6_sneaking_gemstones(account):
    ...
    gemstone_multi = lava_func("decayMulti", generational_gemstones_level, 3, 300)
    account.sneaking.calculate_gemstone_value(
...
def _calculate_w6_farming_crop_value(account):
    ...
    value_multi = {}
    value_multi['Doubler Multi'] = floor(farming.market['Product Doubler'].as_multi)
    value_multi['Mboost Sboost Multi'] = ValueToMulti(
        farming.land_rank['Production Megaboost'].value
        + farming.land_rank['Production Superboost'].value
    )
    value_multi['Value GMO Current'] = farming.market['Value Gmo'].as_multi

    # Ballot Buff * Active status
    ballot_value = account.ballot['Buffs'][29]['Value'] * int(account.ballot['CurrentBuff'] == 29)
    # Calculate with the Min Plot Rank
    value_multi['Pboost Ballot Multi Min'] = ValueToMulti(
        # Value of PBoost * Lowest Plot Rank
        farming.land_rank['Production Boost'].value * farming.land_rank.min_level
        + ballot_value
    )
    ...
    value_multi['Pboost Ballot Multi Max'] = ValueToMulti(
        ...
        farming.land_rank['Production Boost'].value * farming.land_rank.max_level
        + ballot_value
    )
    ...
    account.farming.multi['Value'] = value_multi
...
def _calculate_w6_farming_crop_evo(account):
        ...
        for mapIndex in range(251, 264):  # Clearing the fake portal at Samurai Guardians doesn't count
            try:
                if int(float(char.kill_dict.get(mapIndex, [1])[0])) <= 0:
                    evo_multi['Maps Opened'] += 1
                    farming.magic_bean_unlocked = (
                        farming.magic_bean_unlocked or mapIndex == 257
                    )
            except:
                continue
    evo_multi['Cropius Final Value'] = evo_multi['Maps Opened'] * account.alchemy_bubbles['Cropius Mapper']['BaseValue']
    ...
    # Stamp
    evo_multi['Stamp Multi'] = ValueToMulti(account.stamps['Crop Evo Stamp'].total_value)
    # Meals
    evo_multi['Nyan Stacks'] = ceil((max(account.all_skills['Summoning'], default=0) + 1) / 50)
    evo_multi['Meals Multi'] = (
        ValueToMulti(account.meals['Bill Jack Pepper']['Value'])
        * ValueToMulti(account.meals['Nyanborgir']['Value'] * evo_multi['Nyan Stacks'])
    )
    # Markets
    evo_multi['Farm Multi'] = (
        farming.market['Biology Boost'].as_multi
        * farming.market['Evolution Gmo'].as_multi
    )
    # Land Ranks
    evo_multi['LR Multi'] = (
            ValueToMulti(farming.land_rank['Evolution Boost'].value * farming.land_rank.min_level)
            * ValueToMulti(farming.land_rank['Evolution Megaboost'].value)
            * ValueToMulti(farming.land_rank['Evolution Superboost'].value)
            * ValueToMulti(farming.land_rank['Evolution Ultraboost'].value)
    )
    # Starsign
    ...
    evo_multi['Skill Mastery Bonus Bool'] = account.rift['SkillMastery'] and evo_multi['Total Farming Levels'] >= 300
    ...
    evo_multi['Ballot Multi Max'] = ValueToMulti(account.ballot['Buffs'][29]['Value'])
    evo_multi['Ballot Multi Current'] = max(1, evo_multi['Ballot Multi Max'] * evo_multi['Ballot Active'])
    evo_multi['Misc Multi'] = (
            ValueToMulti(5 * account.achievements["Lil' Overgrowth"]['Complete'])
            * account.killroy_skullshop['Crop Multi']
            * ValueToMulti(15 * evo_multi['Skill Mastery Bonus Bool'] * account.rift['SkillMastery'])
            * evo_multi['Ballot Multi Current']
    )
    evo_multi['Wish Multi'] = ValueToMulti(account.caverns['Caverns']['The Lamp']['WishTypes'][8]['BonusList'][0])
    ...
    account.farming.multi['Evo'] = evo_multi
...
def _calculate_w6_farming_crop_speed(account):
    ...
    speed_multi['VM Multi'] = ValueToMulti(
        speed_multi['Vial Value']
        + farming.market['Nutritious Soil'].value
    )
    # Night Market
    speed_multi['NM Multi'] = farming.market['Speed Gmo'].as_multi
    # Total
    ...
    account.farming.multi['Speed'] = speed_multi


def _calculate_w6_farming_bean_bonus(account):
    farming = account.farming
    bean_multi = {}
    bean_multi['mga'] = farming.market['More Beenz'].as_multi
    ...
    account.farming.multi['Bean'] = bean_multi
...
def _calculate_w6_farming_og(account):
    ...
    og_multi['NM Multi'] = farming.market['Og Fertilizer'].as_multi
    og_multi['Merit Multi'] = ValueToMulti(2 * account.merits[5][2]['Level'])
    og_multi['LR Multi'] = ValueToMulti(
        farming.land_rank['Overgrowth Boost'].value
        + farming.land_rank['Overgrowth Megaboost'].value
        + farming.land_rank['Overgrowth Superboost'].value
    )
    og_multi['Pristine Multi'] = ValueToMulti(
        account.sneaking.pristine['Taffy Disc'].value
    )
    ...
    account.farming.multi['OG'] = og_multi
