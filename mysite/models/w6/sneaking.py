...
class Sneaking:
    ...
    def _parse_gemstones(self, raw_optlacc):
        for name, info in gemstones_info.items():
            ...
            self.gemstone[name] = Gemstone(name, info, level)

    def calculate_gemstone_value(self, talent_level, gemstone_multi):
        moissanite = self.gemstone["Moissanite"]
        moissanite.calculate_value(None, 1.0)
        self.gemstone_multi_source = (talent_level, gemstone_multi)
        for gemstone in self.gemstone.values():
