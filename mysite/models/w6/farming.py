...
from consts.w6.farming import max_land_rank_level, max_farming_crops, crop_dict
...
class Market:
    ...
    def get_bonus_advice(self, show_cost: bool = False):
        ...
        if self._stack is not None:
            label += f"<br>{self._stack} stacks = {self._stack_value:,g}x"
        return Advice(
            label=label,
...
class LandRankUpgrade:
    ...
    def get_bonus_advice(
        ...
        if self.max_level is not None and self.level < self.max_level:
            progress = f"{round_and_trim(self.value)}/{round_and_trim(self.max_value)}"
        else:
...
class LandRank(dict[str, LandRankUpgrade]):
    ...
    def get_bonus_with_land_rank_advice(self, name: str) -> list[Advice]:
        ...
        upgrade_advice.label += (
            ...
            f"<br>Total on Highest: +{round_and_trim(upgrade.value * self.max_level)}%"
        )
