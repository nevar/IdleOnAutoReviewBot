...
def getEvoChanceAdviceGroup(farming: Farming, highest_farming_level) -> AdviceGroup:
    ...
    alch = f"Alchemy: {farming.multi['Evo']['Alch Multi']:.3f}x"
    stamp = f"Stamps: {farming.multi['Evo']['Stamp Multi']:.3f}x"
    meals = f"Meals: {farming.multi['Evo']['Meals Multi']:.3f}x"
    farm = f"Markets: {farming.multi['Evo']['Farm Multi']:.3g}x"
    lr = f"Land Ranks: {farming.multi['Evo']['LR Multi']:,.3f}x"
    summon = f"Summoning: {round_and_trim(summoning_evo)}x"
    ss = f"Star Sign: {farming.multi['Evo']['SS Multi']:.3f}x"
    lamp = f"Lamp Wish: {farming.multi['Evo']['Wish Multi']:.3f}x"
    misc = f"Misc: {farming.multi['Evo']['Misc Multi']:.3f}x"
    total = f"Total Evo Chance: {farming.multi['Evo']['Subtotal Multi']:.3g}x"
    evo_advices = {
        total: [],
        alch: [],
        stamp: [],
...
#Bubbles
    ...
    evo_advices[alch].append(Advice(
        label=f"Cropius Mapper: {farming.multi['Evo']['Maps Opened']}/{max_characters * (len(session_data.account.enemy_maps[6]) - 1)} maps"
              f"<br>Total value: {farming.multi['Evo']['Cropius Final Value']:.3f}%",
...
#Vial
    evo_advices[alch].append(Advice(
        label=f"{{{{ Vial|#vials }}}}: Flavorgil (Caulifish): {session_data.account.alchemy_vials['Flavorgil (Caulifish)']['BaseValue']:.2f}%"
              f"<br>Total Value after multis: {farming.multi['Evo']['Vial Value']:.2f}%",
...
#Meals
    ...
    evo_advices[meals].append(Advice(
        label=f"Highest Summoning level: {max(session_data.account.all_skills['Summoning'], default=0)}"
              f"<br>Provides a {farming.multi['Evo']['Nyan Stacks']}x multi to Nyanborgir",
        picture_class='summoning'
    ))
    evo_advices[meals].append(Advice(
        label=f"{{{{ Meal|#cooking }}}}: Nyanborgir: {session_data.account.meals['Nyanborgir']['Description']}"
              f"<br>After {farming.multi['Evo']['Nyan Stacks']} Summoning Level stack{pl(farming.multi['Evo']['Nyan Stacks'])}:"
              f" {session_data.account.meals['Nyanborgir']['Value'] * farming.multi['Evo']['Nyan Stacks']:,.3f}%",
...
#Star Sign
    ...
    evo_advices[ss].append(Advice(
        label=f"{{{{ Starsign|#star-signs }}}}: Cropiovo Minor: {3 * session_data.account.star_signs['Cropiovo Minor']['Unlocked']:.0f}/3% per farming level."
              f"<br>Total Value if doubled: {farming.multi['Evo']['Starsign Final Value']:,.3f}%",
...
# MISC
    ...
    evo_advices[misc].append(Advice(
        label=f"Skill Mastery at 200 Farming: +{1.15 * farming.multi['Evo']['Skill Mastery Bonus Bool'] * session_data.account.rift['SkillMastery']}/1.15x",
        picture_class='farming',
        progression=farming.multi['Evo']['Total Farming Levels'],
        goal=200
    ))

    evo_advices[misc].append(Advice(
        label=f"Weekly Ballot: {farming.multi['Evo']['Ballot Multi Current']:.3f}/{farming.multi['Evo']['Ballot Multi Max']:.3f}x"
              f"<br>(Buff {farming.multi['Evo']['Ballot Status']})",
        picture_class='ballot-29',
        progression=int(farming.multi['Evo']['Ballot Active']),
        goal=1
    ))

#Total
    target_evo_crop = None
    for crop_index in crop_evo_breakpoint_list:
        crop_evo_chance = ceil(1 / farming.crops.evo_chance(crop_index))
        if farming.multi['Evo']['Subtotal Multi'] > crop_evo_chance:
    ...
    if target_evo_crop is not None:
        ...
        precent = farming.multi['Evo']['Subtotal Multi'] / crop_evo_chance
